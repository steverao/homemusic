...
$(function(){
	...
	$("#user-login").click(function(){
		...
		var status = window.loginStatus;
		if(status == "1"){
			swal({title:"您已登陆！",text:"It wills close in 2 seconds later!",type:"error", timer:2000});
		}else{
			$("#login").fadeIn(1000);
		}
	});
					...
					success:function(dataStr){
						...
						$(data).each(function(index){
							...
							var $btn = "<div class='glyphicon glyphicon-play-circle  play-btn'>"
											+"<div style='display:none' class='sid'>"+data[index].id+"</div>"
										+"</div>";
							var $coll = "<td class='img-wrap'>"
											+"<div class='glyphicon glyphicon-star-empty add-song coll-song'>"
											+"<div style='display:none' class='sid'>"+data[index].id+"</div>"
											+"</div></td>"
							$(".search-cont").append("<tr class='scl-item'>"
														+"<td class='img-wrap'>"
															+"<div class='song-img'>"
																+"<img src='"+data[index].img+"' class='img-thumbnail' />"
															+"</div>"
															+$btn
														+"</td>"
														+songName
		...
		function login(){
			...
			$.ajax({
				...
				success:function(data){
					swal({title:"登陆成功！",text:"It wills close in 2 seconds later!",type:"success", timer:2000});
				}
			});
		}
									...
									$(data).each(function(index){
										...
										var pbtn = "<td class='img-wrap'>"
													...
													+"<div class='play-btn'>"
														...
														+"<div style='display:none' class='lsid'>"+data[index].id+"</div>"
													+"</div>"
													+"</td>";
										...
										$(".del-btn").on("click", function(){
											...
											swal({
												...
												$.ajax({
													...
													async:false,
													success:function(data){
														swal({title:"已删除!",text:"你已删除！",type:"success", timer:1000});
													}
												});
												getUserLst();
											});
		...
		function displayLst(lsid){
			...
			$.ajax({
				...
				success:function(data){
					...
					var i = 1;
					$(data).each(function(index){
						var sid = "<div style='display:none' class='sid'>"+data[index].id+"</div>";
						var num = "<td>"+sid+(i++)+".</td>";
						var song = "<td class='opt-song'>"
										+"<div class='opt-song-name'>"+data[index].name+"</div>"
										+"<div class='cont-opt'>"
												...
												+"</div>"
									+"</td>";
						var singer = "<td>"+data[index].singer+"</td>";
						$cnt.append("<tr>"
										+num
										+song
										+singer
					...
					$(".del-song").on("click", function(){
						var slid = $("#ls-id").text();
						var sid = $(this).parent().parent().siblings().eq(0).children(".sid").text();
				...
				success:function(data){
					...
					$(data).each(function(index){
						var img = "<td class='img-wrap'>"
										...
										+"<div class='play-btn'>"
											+"<i class='fa fa-play-circle-o' aria-hidden='true'></i>"
											+"<div style='display:none' class='spid'>"+data[index].id+"</div>"
										+"</div>"
								  +"</td>";
						var spname = "<td class='spname-wrap'>"
											+"<div class='special-name'>"
												+data[index].singer+"-"+data[index].name
											+"</div>"
									 +"</td>";
		...
		success:function(data){
			...
			$(data).each(function(index){
				var pbtn = "<td class='img-wrap'>"
							...
							+"</td>"; 
				var name ="<td class='spname-wrap lst-cont'>"
								+"<div style='display:none' class='slid'>"+data[index].id+"</div>"
								+"<div class='special-name'>"
									+data[index].name
								+"</div>"
							+"</td>";
			...
			$(".lst-cont").on("click", function(){
				var spid = $(this).parents(".content").find(".spid").text();
				var sid = $(this).parents(".content").find(".song-id").text();
				...
				//$(".close-flip").click();
				$(this).parents(".collect-container").children(".close-flip").click();
			});
		}
	});
}
...
function getSongsBySlidOrSpid(slid, spid){
	//alert("slid"+slid+" spid"+spid);
	$.ajax({
		type:"post",
		url:"getsongsbyslidorspid.do",
		async:false,
		dataType:"json",
		data:{"slid":slid, "spid":spid},
		success:function(data){
			//alert(data);
			data = eval(data);
...
function displayPlayLst(){
	...
	$(arr).each(function(index){
		var sid = "<div style='display:none' class='sid'>" + arr[index].id + "</div>";
		var num = "<td>" + sid + (i++) + ".</td>";
		var song = "<td class='opt-song'>" +
			"<div class='opt-song-name'>" + arr[index].name + "</div>" +
			"<div class='cont-opt'>" +
			"<span class='glyphicon glyphicon-star-empty span-style coll-song'></span>" +
			"<span class='glyphicon glyphicon-trash span-style del-song'></span>" +
			"</div>" +
